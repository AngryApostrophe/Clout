...
class CommsConsole
{
	...
	// Portable helpers
	static int   Stricmp(const char* s1, const char* s2) { int d; while ((d = std::toupper(*s2) - toupper(*s1)) == 0 && *s1) { s1++; s2++; } return d; }
	...
	{
		CommsConsole* console = (CommsConsole*)data->UserData;
